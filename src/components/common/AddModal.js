...
function AddModal(props) {
    ...
    const Changetheme = (color) => {
        const list = document.getElementById('medingen-layout')
        list.className = `theme-${color}`
        setArth({ arth: !arth })
    }

    const Isactivetheme = (val) => {
        var themeA = document.getElementById('medingen-layout');
        if (themeA) {
            if (document.getElementById('medingen-layout').classList.contains('theme-' + val)) {
                return 'active'
            }
        }
        return ''
